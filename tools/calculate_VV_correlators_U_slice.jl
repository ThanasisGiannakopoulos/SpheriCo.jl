...
# initiate VV correlators
VV_crlt = zeros(ComplexF64, (NV, NV))
...
@threads for l in 1:lmax
    ...
    for k in 1:kmax
        ...
        buffers[:,:,id] +=  dk*(
            ...
            uq_UV[ui,:,4].*transpose(conj.(uq_UV[ui,:,4]))
        )
     end # end k loops
end # end l loops; has @threads

VV_crlt[:,:] .= sum(buffers[:,:,:], dims=3)

println()
println("Δt = ", time() - t1)
GC.gc()

h5write(your_dir*"/data_$(Nr)/VV_crlt_NV$(NV)_ui$(ui).h5", "crlt", VV_crlt)
